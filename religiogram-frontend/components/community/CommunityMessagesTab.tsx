          ...
          <button key={t.threadId} onClick={() => setOpenPeer(t.peer)} style={{
            ...
            {t.peer.avatarUrl ? (
              <div style={{
                ...
                background: `center/cover url('${t.peer.avatarUrl}')`,
                flexShrink: 0,
              }} />
            ) : (
              <div style={initialsAvatarStyle(46)}>{getInitials(t.peer.name, t.peer.username)}</div>
            )}
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: '#fff', borderBottom: '1px solid rgba(200,146,10,0.18)' }}>
        ...
        {peer.avatarUrl ? (
          ...
          <div style={initialsAvatarStyle(36)}>{getInitials(peer.name, peer.username)}</div>
        )}
        <div style={{ flex: 1, minWidth: 0 }}>
