...
export default function HomeScreen() {
  ...
  /* Religion-aware Home rails: two horizontal-scroll sections (Priests
   * and Holy Places) that re-fetch whenever the picked religion changes.
   * When religion is null (never picked) we render a lightweight CTA
   * instead of the rail; when it is 'all' we drop the religion query
   * param so the backend returns the pan-faith default. Failure is
   * silent so a bad network never breaks Home. */
  const { religion, loaded: religionLoaded } = useReligion();

  interface HomePriest { id: string; name: string; photo: string; rating: number; religion: string | null; fee: number; }
  interface HomePlace  { id: string; name: string; city: string; photo: string; type: string; }

  const [priests,      setPriests]      = useState<HomePriest[]>([]);
  const [priestsLoad,  setPriestsLoad]  = useState(false);
  const [places,       setPlaces]       = useState<HomePlace[]>([]);
  const [placesLoad,   setPlacesLoad]   = useState(false);

  useEffect(() => {
    if (!religionLoaded || religion === null) return;
    const controller = new AbortController();
    const tok = tokenStore.access ?? '';
    const headers: Record<string, string> = {};
    if (tok) headers['Authorization'] = 'Bearer ' + tok;

    // Backend uses 'islam' for Muslim rows (see ProviderMarketplace.tsx).
    const priestRel = religion === 'muslim' ? 'islam' : religion;

    // Priests
    setPriestsLoad(true);
    const priestParams = new URLSearchParams({ category: 'priest', limit: '6' });
    if (religion !== 'all') priestParams.set('religion', priestRel);
    fetch(`${API_BASE}/providers?${priestParams.toString()}`, { headers, signal: controller.signal })
      .then(r => r.ok ? r.json() : null)
      .then(j => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const raw: any[] = Array.isArray(j) ? j : (j?.items ?? j?.data ?? []);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        setPriests(raw.slice(0, 6).map((p: any): HomePriest => ({
          id:       String(p.id ?? p.providerId ?? ''),
          name:     String(p.fullName ?? p.name ?? 'Priest'),
          photo:    String(p.avatarUrl ?? p.photoUrl ?? ''),
      ...
      .catch(() => { /* silent */ })
      .finally(() => { if (!controller.signal.aborted) setPriestsLoad(false); });

    // Holy Places
    setPlacesLoad(true);
    const placesParams = new URLSearchParams({ limit: '6' });
    if (religion !== 'all') placesParams.set('religion', religion);
    fetch(`${API_BASE}/places?${placesParams.toString()}`, { headers, signal: controller.signal })
      .then(r => r.ok ? r.json() : null)
      .then(j => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const raw: any[] = Array.isArray(j) ? j : (j?.data ?? j?.places ?? j?.items ?? []);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        setPlaces(raw.slice(0, 6).map((p: any): HomePlace => ({
          id:    String(p.id ?? p._id ?? ''),
          name:  String(p.name ?? 'Holy Place'),
          city:  String(p.city ?? p.address ?? p.location ?? ''),
          photo: String(p.imageUrl ?? p.photoUrl ?? ''),
          type:  String(p.type ?? p.placeType ?? 'temple'),
        })));
      })
      .catch(() => { /* silent */ })
      .finally(() => { if (!controller.signal.aborted) setPlacesLoad(false); });

    return () => controller.abort();
  }, [religion, religionLoaded]);

  const capitalize = (s: string) => s ? s.charAt(0).toUpperCase() + s.slice(1) : s;
  const priestsHref = (religion && religion !== 'all') ? `/priests?faith=${religion}` : '/priests';
  const priestsTitle = (religion && religion !== 'all')
    ? `Priests for ${capitalize(religion)}`
    : 'Verified Priests';
  const placesTitle = (religion && religion !== 'all')
    ? `Holy Places for ${capitalize(religion)}`
    : 'Holy Places near you';
  ...
  return (
    <div style={{ minHeight: '100svh', background: BG, fontFamily: '"Plus Jakarta Sans", sans-serif', paddingBottom: 30 }}>
      ...
      {/* ── EXPLORE PRIESTS (religion-aware horizontal rail) ──────────────
       * Deep-links each card + the "See all" link to /priests?faith=<rel>
       * so the Priests screen skips its own faith picker. When the user
       * hasn't chosen a religion yet, a compact CTA nudges them to /profile
       * where the picker lives (also reachable from the bottom nav).      */}
      <div style={{ margin: '22px 16px 0' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 12 }}>
          <span style={{ fontSize: 17, fontWeight: 800, color: TEXT, fontFamily: '"Playfair Display",Georgia,serif' }}>
            {priestsTitle}
          </span>
          {religion !== null && (
            <Link href={priestsHref} style={{ fontSize: 12, fontWeight: 700, color: GOLD, textDecoration: 'none' }}>
              See all &#x2192;
            </Link>
          )}
        </div>
        ...
        {religion === null ? (
          <Link href="/profile" style={{
            ...
            border: '1.5px solid rgba(200,146,10,0.28)',
            boxShadow: '0 2px 8px rgba(0,0,0,0.06)',
            padding: '14px 16px',
          }}>
            <div style={{ fontSize: 13.5, fontWeight: 700, color: NAVY, marginBottom: 4 }}>
              Pick your faith to personalize
            </div>
            <div style={{ fontSize: 12, color: TEXT2, lineHeight: 1.5 }}>
              Tap to choose your religion and see priests & places tailored to you.
            </div>
          </Link>
        ) : priestsLoad ? (
          <div style={{ display: 'flex', overflowX: 'auto', scrollbarWidth: 'none', gap: 12 }}>
            {[0,1,2,3].map(i => (
              <div key={i} style={{
                flex: '0 0 160px', background: '#fff', borderRadius: 14,
                border: '1px solid rgba(200,146,10,0.18)',
                boxShadow: '0 2px 8px rgba(0,0,0,0.05)',
                padding: 12, minHeight: 176,
              }}>
                <div style={{ width: 56, height: 56, borderRadius: '50%', background: '#F1E7CE', margin: '0 auto 10px' }} />
                <div style={{ height: 10, background: '#F1E7CE', borderRadius: 4, margin: '0 0 6px' }} />
                <div style={{ height: 8, background: '#F5EED8', borderRadius: 4, width: '70%', margin: '0 auto' }} />
              </div>
            ))}
          </div>
        ) : priests.length === 0 ? (
          <div style={{
            background: '#fff', borderRadius: 14,
            border: '1.5px solid rgba(200,146,10,0.2)',
            padding: '18px 16px', fontSize: 12.5, color: TEXT3, textAlign: 'center',
          }}>
            No priests yet for this faith. Check back soon.
          </div>
        ) : (
          <div style={{ display: 'flex', overflowX: 'auto', scrollbarWidth: 'none', gap: 12, paddingBottom: 4 }}>
            {priests.map(p => (
              <Link key={p.id} href={priestsHref} style={{
                ...
                {p.photo ? (
                  ...
                  <img
                    src={p.photo}
                    alt={p.name}
                    style={{ width: 56, height: 56, borderRadius: '50%', objectFit: 'cover', marginBottom: 8, background: '#F1E7CE' }}
                    onError={(e) => { (e.currentTarget as HTMLImageElement).style.visibility = 'hidden'; }}
                  />
                ) : (
                  <div style={{
                    width: 56, height: 56, borderRadius: '50%', background: NAVY, color: GOLD_L,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                ...
                <div style={{
                  fontSize: 13, fontWeight: 700, color: TEXT, textAlign: 'center',
                  lineHeight: 1.25, marginBottom: 4,
                  display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden',
                }}>
                  {p.name}
                </div>
                <div style={{ fontSize: 10.5, color: TEXT3, marginBottom: 6, textTransform: 'capitalize' }}>
                  {p.religion ? String(p.religion) : 'Priest'}
                </div>
                {p.rating > 0 && (
                  <div style={{ marginBottom: 6 }}>
                    <StarRating rating={Number(p.rating.toFixed(1))} />
                  </div>
                )}
                <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
                  ...
                  <span style={{
                    fontSize: 10.5, fontWeight: 800, color: NAVY, background: GOLD_L,
      ...
      {/* ── EXPLORE HOLY PLACES (religion-aware horizontal rail) ────────── */}
      <div style={{ margin: '22px 16px 0' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 12 }}>
          <span style={{ fontSize: 17, fontWeight: 800, color: TEXT, fontFamily: '"Playfair Display",Georgia,serif' }}>
            {placesTitle}
          ...
          {religion !== null && (
            <Link href="/places" style={{ fontSize: 12, fontWeight: 700, color: GOLD, textDecoration: 'none' }}>
              See all &#x2192;
            </Link>
          )}
        </div>
        ...
        {religion === null ? (
          <Link href="/profile" style={{
            ...
            <div style={{ fontSize: 12, color: TEXT2, lineHeight: 1.5 }}>
              Tap to choose your religion and see holy places tailored to you.
            </div>
          </Link>
        ) : placesLoad ? (
          <div style={{ display: 'flex', overflowX: 'auto', scrollbarWidth: 'none', gap: 12 }}>
            {[0,1,2,3].map(i => (
              <div key={i} style={{
                flex: '0 0 168px', background: '#fff', borderRadius: 14,
                border: '1px solid rgba(200,146,10,0.18)',
                boxShadow: '0 2px 8px rgba(0,0,0,0.05)',
                overflow: 'hidden', minHeight: 178,
              }}>
                <div style={{ width: '100%', height: 100, background: '#F1E7CE' }} />
                <div style={{ padding: 10 }}>
                  <div style={{ height: 10, background: '#F1E7CE', borderRadius: 4, margin: '0 0 6px' }} />
                  <div style={{ height: 8, background: '#F5EED8', borderRadius: 4, width: '60%' }} />
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{
            ...
            No holy places yet for this faith. Check back soon.
          </div>
        ) : (
          <div style={{ display: 'flex', overflowX: 'auto', scrollbarWidth: 'none', gap: 12, paddingBottom: 4 }}>
            {places.map(pl => (
              <Link key={pl.id} href={`/places/${pl.id}`} style={{
                flex: '0 0 168px', textDecoration: 'none',
                background: '#fff', borderRadius: 14,
                border: '1px solid rgba(200,146,10,0.28)',
                boxShadow: '0 2px 8px rgba(0,0,0,0.06)',
                ...
                <div style={{ position: 'relative', width: '100%', height: 100, background: '#F1E7CE' }}>
                  {pl.photo ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img
                      src={pl.photo}
                ...
                <div style={{ padding: 10, display: 'flex', flexDirection: 'column', flex: 1 }}>
                  <div style={{
                    fontSize: 12.5, fontWeight: 700, color: TEXT, lineHeight: 1.25,
                    display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden',
                    marginBottom: 4,
                  }}>
                    {pl.name}
                  ...
                  <div style={{
                    fontSize: 10.5, color: TEXT3, textTransform: 'capitalize',
