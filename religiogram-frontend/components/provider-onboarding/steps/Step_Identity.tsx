...
/**
 * Step — Identity Documents (shared: priest, astrologer, both).
 *
 * Two photo uploads:
 *   - PAN card (rear-facing camera or file pick)
 *   - Selfie (front camera or file pick)
 *
 * Same presign → S3 PUT → confirm pattern as the KYC video step. Each card
 * is independent — one can succeed while the other is still pending — and
 * the Continue button only enables once both confirmations have come back.
 */
...
interface Props {
  flow: FlowConfig;
  /** Full path to the payout step for router.push on continue. */
  nextStepPath: string;
  /** Callback returning a redirect path if prerequisites are missing. */
  gateCheck: (data: Record<string, any>) => string | null;
}

export default function Step_Identity({ flow, nextStepPath, gateCheck }: Props) {
