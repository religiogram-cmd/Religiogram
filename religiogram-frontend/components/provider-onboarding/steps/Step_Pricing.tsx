...
/**
 * Step — Per-booking pricing (priest + both flows).
 */
...
interface Props {
  flow: FlowConfig;
  /** Path to the faith step for gating fallback. */
  faithStepPath: string;
  /** Path to the services step for gating fallback. */
  servicesStepPath: string;
}

export default function Step_Pricing({ flow, faithStepPath, servicesStepPath }: Props) {
  ...
  useEffect(() => {
    if (!data.religion) {
      router.replace(faithStepPath);
      return;
    }
    const picked = (data.selectedServiceIds?.length ?? 0) + (data.customServiceNames?.length ?? 0);
    if (picked === 0) {
      router.replace(servicesStepPath);
    }
  }, [data.religion, data.selectedServiceIds, data.customServiceNames, router, faithStepPath, servicesStepPath]);
