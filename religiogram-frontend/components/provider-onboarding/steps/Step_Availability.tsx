...
/**
 * Step — Weekly availability (shared: priest, astrologer, both).
 *
 ...
 * the whole point.
 *
 * Copy varies per flow:
 *   · priest / both → "when you'll do in-person visits + accept bookings"
 *   · astrologer    → "when you'll be online to take chat/voice/video calls"
 *
 * We accept two copy props (heading paragraph + micro-copy on "day off")
 * so the pricing/mode gating logic can stay shared.
 *
 * Gating: priest needs prior faith + pricing; astrologer only needs prior
 * per-minute rate. The `gates` prop lets each caller declare what to check.
 */
...
interface Props {
  flow: FlowConfig;
  /** Header paragraph shown above the day list. */
  intro: string;
  /** Whether to gate on faith + pricing (priest / both) or on perMinutePaise
   *  (astrologer). Gating failures redirect to `gateFallbackPath`. */
  gateStrategy: 'priestServices' | 'astrologerPerMinute';
  /** Where to bounce a user who lands here without the prerequisites. */
  gateFallbackPath: string;
}

export default function Step_Availability({
  flow,
  intro,
  gateStrategy,
  gateFallbackPath,
  ...
  useEffect(() => {
    if (gateStrategy === 'priestServices') {
      if (!data.religion || !data.pricing?.length) {
        router.replace(gateFallbackPath);
      }
    } else {
      if (!data.perMinutePaise || data.perMinutePaise <= 0) {
        router.replace(gateFallbackPath);
      }
    }
  }, [data.religion, data.pricing, data.perMinutePaise, router, gateStrategy, gateFallbackPath]);
  ...
  return (
    ...
    >
      <div className="space-y-4">
        <p className="text-sm text-gray-700/80">{intro}</p>
