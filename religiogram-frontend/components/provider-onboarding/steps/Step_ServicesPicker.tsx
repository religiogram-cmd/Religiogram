...
/**
 * Step — Services selection (priest + both flows).
 *
 * Loads the religion-scoped catalogue and renders it as collapsible
 * categories with multi-select checkboxes. The "Other service" input at
 * the bottom lets providers add up to 10 custom services that aren't in
 * the master list — they flow through provider_services.custom_name
 * server-side.
 *
 * If the user lands on this screen with no religion set (e.g. deep-link
 * or stale tab), we redirect back to the faith step of the SAME flow.
 * We hand-wire that path via `faithStepPath` because the priest flow puts
 * faith on step 3 while the both flow puts it on… also step 3, but the
 * both flow has a longer trailing tail and we want the routing coupled to
 * the flow config, not baked into the component.
 */
...
interface Props {
  flow: FlowConfig;
  /** Full path to the faith step for gating fallback. */
  faithStepPath: string;
}

export default function Step_ServicesPicker({ flow, faithStepPath }: Props) {
  ...
  useEffect(() => {
    if (!data.religion) router.replace(faithStepPath);
  }, [data.religion, router, faithStepPath]);
